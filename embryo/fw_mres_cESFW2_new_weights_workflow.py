...
##############################################
def Feature_Weighting_correct(ESS_Threshold, EPs_Threshold, Min_Edges, ESSs, EPs, Used_Features):
    ESSs = np.array(ESSs)
    EPs = np.array(EPs)
    Absolute_ESSs = np.absolute(ESSs)
    print(f'{type(Absolute_ESSs)} is the type of ESS object')
    ## Network filtering
    ...
    EPs_Graph = EPs.copy()
    print(f'{type(EPs_Graph)} is the type of EP graph object')
    ...
    Mask_Inds = np.where((EPs <= EPs_Threshold) & (Absolute_ESSs < ESS_Threshold))
    Masked_ESSs = Absolute_ESSs.copy()# ESSs.copy() #Absolute_ESSs.copy()
    Masked_ESSs[Mask_Inds] = 0
    Masked_EPs = EPs.copy()
    Masked_EPs[Mask_Inds] = 0
    
    Mask_Inds = np.where((EPs <= EPs_Threshold))
    Masked_ESSs[Mask_Inds] = 0
    Masked_EPs[Mask_Inds] = 0
    #also mask all EPs
    ...
    print(f'{type(Normalised_Network_Feature_Weights)} is the type of normalised_weight object')
    
    w = pd.DataFrame(data=Feature_Weights)
    w.to_csv('cESFW_cESFW-subset_weights_newweights.csv')

    nw = pd.DataFrame(data=Normalised_Network_Feature_Weights)
    nw.to_csv('cESFW_cESFW-subset_normalised_weights_newweights.csv')

    feat = pd.DataFrame(data=Used_Features)
    feat.to_csv('cESFW_cESFW-subset_used_feature_weights_newweights.csv')
...
def Plot_UMAP(Embedding,Sample_Info, save_name):
    ...
    plt.savefig(f"timepoints_{save_name}_arthur_workflow_withmt_newweights.png",dpi=600)
    #
    plt.figure(figsize=(7,7))
    ...
    plt.xlabel('UMAP 1',fontsize=14)
    plt.ylabel('UMAP 2',fontsize=14)
    plt.savefig(f"datasets_{save_name}_arthur_workflow_withmt_newweights.png",dpi=600)
    #
    plt.figure(figsize=(7,7))
    ...
    plt.xlabel('UMAP 1',fontsize=14)
    plt.ylabel('UMAP 2',fontsize=14)
    plt.savefig(f"cellannot_{save_name}_arthur_workflow_withmt_newweights.png",dpi=600)
...
Subset_Used_Features, Sub_ESSs, Sub_EPs, ESSs_Graph, EPs_Graph, Masked_ESSs, Masked_EPs, Feature_Weights, Normalised_Network_Feature_Weights, Significant_Genes_Per_Gene = Feature_Weighting_correct(ESS_Threshold, EPs_Threshold, Min_Edges, np.asarray(ESSs), np.asarray(EPs), Used_Features)

### Feature selection importance weighting ###

## Some genes that we know are important in early human embryo development
Known_Important_Genes = np.array(["NANOG","SOX17","GATA4","PDGFRA","PRSS3","SPIC", "SOX2", "CNPY4"])
for gene in Known_Important_Genes:
    Known_Important_Gene_Inds = np.where(np.isin(Subset_Used_Features,gene))[0]
...
plt.xlabel("UMAP 1",fontsize=16)
plt.ylabel("UMAP 2",fontsize=16)
plt.savefig("Gene_Cluster_arthur_workflow_withmt_newweights.png",dpi=600)
# plt.close()
#plt.show()
